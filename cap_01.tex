...
\begin{figure}[H]
	...
	\caption{An example of one of  \matlab stock images using a large CNN pre-trained on ImageNet [\sref{s:imagenet}].}
	\label{f:demo}
\end{figure}
...
\subsubsection{Overfitting}\label{s:trainalg-overfitting}
\begin{figure}[H]
	\centering
...
\subsubsection{Evaluation Metrics}\label{s:trainalg-evalmetrics}
...
So for example if most of the items I retrieve with a query are good results, I will get good \emph{Precision}. But if the results my query retrieved are only a fraction of the total good results in the dataset, then I will have a bad \emph{Recall} score.
...
COCO is a dataset that offers state-of-the-art standardized evaluation metrics to evaluate the results of a particular Neural Network for analyzing images' contents.
...
You can see this at work in \sref{s:parameters-and-tests}.
...
% ------------------------------------------------------------------
\subsubsection{Image Preprocessing}\label{s:imgpr}
% ------------------------------------------------------------------
...
The classification loss is used in our implementation, described in \sref{s:fizyr}.
...
Refer to \sref{s:loss-multi}. $L_\textrm{box}$ is defined as the loss associated with the bounding boxes: their offset relative to an object proposal.
...
As described in the intro to CNNs, \sref{s:back}, Backpropagation is an essential element for training NNs because it is used in RoIAlign, \sref{s:roi-align}, in the convolution. That is how it updates the weights using gradient descent.
